...
- **Upstream proxy**: Optional name of an instance-level [upstream proxy](/learn/upstream-proxies) profile that carries this service's requests to the real API. Empty means "instance default, else dial the target directly".
...
By default Agent Vault dials the target directly. Reference an [upstream proxy](/learn/upstream-proxies) profile to send the request through an operator-configured egress hop instead:
...
The value is a profile name managed by instance owners (**Manage Instance > Upstream Proxies**), never an inline URL — credentials and trust anchors live on the profile, so rotating egress for ten services is one edit. A reference to a profile that does not exist is rejected with `400` at write time, and deleting a profile while services still reference it is refused with `409`.

<Tip>
  Agents cannot change egress routing. A [proposal](/learn/proposals) that adds
  or edits a service keeps whatever `upstream_proxy` is already stored, so an
  agent can neither introduce nor drop routing for a service it does not own.
</Tip>
